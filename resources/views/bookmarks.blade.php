            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
                @foreach ($bookmarkedHotels as $bookmarkedHotel)
                    <div class="cursor-pointer rounded-xl bg-white p-3 shadow-lg hover:shadow-xl>
                        <div class="relative
                        flex items-end overflow-hidden rounded-xl"
                        onclick="window.location.href = '{{ route('room.view', $bookmarkedHotel->room_id) }}'">
                        @if ($bookmarkedHotel->room->photos->isNotEmpty())
                            @foreach ($bookmarkedHotel->room->photos as $photo)
                    ...
                    <div
                        ...
                        <span class="ml-1 text-sm text-slate-400">{{ $bookmarkedHotel->room->rating }}</span>
                    </div>
                </div>

                <div class="mt-1 p-2">
                    <h2 class="text-slate-700 text-md font-bold">{{ $bookmarkedHotel->room->roomType->name }} room</h2>
                    <p class="mt-1 text-sm text-slate-400">{{ $bookmarkedHotel->room->hotel->name }}</p>
                    <div class="mt-3 flex items-end justify-between">
                        <p>
                            <span class="text-lg font-bold text-primary-500">${{ $bookmarkedHotel->room->price }}</span>
                        ...
                        <div class="group inline-flex rounded-xl bg-primary-100 p-2 hover:bg-primary-200">
                            @if ($bookmarkedHotel->status === 'canceled')
                            ...
                            @else
                                <button type="button" id="deleteButton" data-modal-target="deleteModal"
                                    data-modal-toggle="deleteModal" data-record-id="{{ $bookmarkedHotel->id }}"
