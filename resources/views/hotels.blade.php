...
<x-footer />
<script>
    let searchHotelForm = document.getElementById('searchHotelForm');
    let searchHotelRoute = searchHotelForm.action;
                ...
                if (data.hotels && data.hotels.length > 0) {
                    ...
                    data.hotels.forEach(function(hotel) {
                        html +=
                            '<div class="cursor-pointer rounded-xl bg-white p-3 shadow-lg hover:shadow-xl">';
                        html +=
                            '<div class="relative flex items-end overflow-hidden rounded-xl" onclick="window.location.href = \'' +
                            '{{ route('hotel.view', '') }}' + hotel.id + '\'">';
                        if (hotel.photos.length > 0) {
                            html += '<img class="h-[250px] w-full" src="/storage/' + hotel.photos[0]
                                .path + '" alt="Hotel Photo" />';
                        } else {
                            html +=
                        ...
                        html += '</svg>';
                        html += '<span class="ml-1 text-sm text-slate-400">' + hotel.rating +
                            '</span>';
                        html += '</div>';
                        html += '</div>';
                        html += '<div class="mt-1 p-2">';
                        html += '<h2 class="text-slate-700">' + hotel.name + '</h2>';
                        html += '<p class="mt-1 text-sm text-slate-400">' + hotel.location.city +
                            ', ' + hotel.location.country +
                            '</p>'; // Utilize location.city and location.country
                        html += '<div class="mt-3 flex items-end justify-between">';
                        html += '<p>';
                        html += '<span class="text-lg font-bold text-primary-500">' + hotel
                            .number_of_rooms + '</span>';
                        ...
                        html +=
                            ...
                            hotel.id + '" data-entitytype="hotel">';
                        html +=
                    ...
                    document.getElementById('searchHotelResults').innerHTML = html;
                } else {
                    document.getElementById('searchHotelResults').innerHTML = "<p>No results found</p>";
                }
            })
